...
import { createUsageRequestEvent } from '@/utils/requestEvents';
...
export function BillingDetailsCard({
  ...
  const [onlySelectedCurrencyBillable, setOnlySelectedCurrencyBillable] = useState(false);
  ...
  const rows = useMemo((): BillingDetailRow[] => {
    ...
    return details
      .map((detail, index) => {
        const event = createUsageRequestEvent(detail, index);
        ...
        const missingPricing = cost.missingPricing === true;
        const otherCurrency = Boolean(selectedCurrency && currencySymbol && !missingPricing && currencySymbol !== selectedCurrency);
        return {
          id: event.id,
          timestamp,
          timestampMs,
          ...
          source: event.sourceRaw || event.source,
  ...
  }, [details, i18n.language, pricingByModel, selectedCurrency]);
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      if (onlySelectedCurrencyBillable) {
  ...
  }, [filterEndpoint, filterModel, filterSource, filterStatus, onlySelectedCurrencyBillable, rows, selectedCurrency]);
  ...
  const subtitle = useMemo(() => {
    ...
    const parts: string[] = [
      timeRangeLabel,
      currencyLabel,
      onlySelectedCurrencyBillable
        ? t('billing.billing_details_scope_current_currency', {
            defaultValue: '仅显示当前币种已计费请求'
          })
        : t('billing.billing_details_scope_all_requests', {
            defaultValue: '显示全部请求'
          })
    ];
    if (onlySelectedCurrencyBillable) {
    ...
    return parts.join(' | ');
  }, [onlySelectedCurrencyBillable, missingPricingCount, otherCurrencyCount, selectedCurrency, t, timeRangeLabel]);
  ...
  const renderEmpty = () => {
    ...
    if (onlySelectedCurrencyBillable && !selectedCurrency) return t('billing.select_currency');
    return t('billing.billing_details_no_rows_filtered', { defaultValue: '当前筛选范围下没有当前币种已计费请求，可切换币种或关闭该过滤器。' });
  };
        ...
        <>
          <div className={monitorStyles.logFilters}>
            ...
            <label className={styles.includeOthers}>
              <input type="checkbox" checked={onlySelectedCurrencyBillable} onChange={(e) => setOnlySelectedCurrencyBillable(e.target.checked)} />
              <span>{t('billing.billing_details_only_selected_currency', { defaultValue: '仅显示当前币种已计费请求' })}</span>
            </label>
          ...
          {onlySelectedCurrencyBillable && (missingPricingCount > 0 || otherCurrencyCount > 0) ? (
                    ...
                    <table className={`${monitorStyles.table} ${styles.virtualTable}`}>
                      <tbody>
                        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                          ...
                          const currency = row.currencySymbol as CurrencySymbol;
                              ...
                              <td>
                                {row.missingPricing ? (
                                  ...
                                  <span className={styles.costCell}>
                                    ...
                                    {row.otherCurrency ? renderPill(t('billing.billing_details_other_currency_badge'), billingStyles.pillMuted) : null}
                                  </span>
                                ) : (
                                  '--'
                                )}
